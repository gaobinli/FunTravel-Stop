...
<template>
    <el-form :inline="true" :model="params" class="demo-form-inline">
        <el-form-item label="轮播图名字：">
            <el-input v-model="params.name" placeholder="请输入轮播图名字"></el-input>
        </el-form-item>
    ...
    <el-table :data="dataTable.list" height="550" style="width: 100%;" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="name" label="轮播图名字" />
    ...
    >
        <el-form :inline="false" :model="oneData" ref="dataFormRuleRef" :rules="addRules" class="demo-form-inline">
            <el-form-item label="轮播图名字：" prop="name">
                <el-input v-model="oneData.name" placeholder="请输入轮播图名字"></el-input>
            </el-form-item>
            <el-form-item label="图片地址：" prop="img">
                ...
                >
                    <img v-if="oneData.img" :src="oneData.img" class="avatar" style="width:148px; height:148px" />
...
export default({
    ...
    setup() {
        ...
        let addRules = reactive({
            name: {required: true, message: '请输入轮播图名字', trigger: 'blur'},
            img: {required: true, message: '请输入图片地址', trigger: 'blur'},
            sort: {required: true, message: '请输入排序', trigger: 'blur'},
        })

        function queryData() {
            get('carousel?currentPage=' + dataTable.currentPage
        ...
        function updated() {
            dataFormRuleRef.value.validate((valid) => {
                if (valid) {
                    patch('carousel/' + oneData.id, oneData).then(res => {
        ...
        function updateEnable(row) {
            patch('carousel/' + row.id, {enable: row.enable}).then(res => {
                ElMessage({message: '亲，修改成功', type: 'success'})
            })
        }
        ...
        function add() {
            dataFormRuleRef.value.validate((valid) => {
                if (valid) {
                    post('carousel', oneData).then(res => {
                    oneData.name = ''
                    oneData.img = ''
                    oneData.sort = ''
                    queryData()
