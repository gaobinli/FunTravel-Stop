...
public class OrderController {
    ...
    @ApiOperation(value = "退货", tags = {"order"}, nickname = "returnOrder")
    @ApiResponses(value = {@ApiResponse(code = 200, message = "操作是否成功,000000:成功，否则失败")})
    @PostMapping(value = "/return", produces = {"application/json"})
    public ResultDTO returnOrder(@RequestParam String ids, @RequestParam Integer status) {
        orderService.returnOrder(ids, status);
        return ResultDTO.ok();
    }
}
